% ANALISIS GYRO X (Rotasi Pitch) DENGAN RMS
try
    % Load file .mat
    load('S1P1_G.mat');
    ...
    % --- Ambil data ---
    gyro_x = gyroX;           % Rotasi pitch (sumbu X)
    t = timestamps;           % Waktu
    ...
    rms_values = sqrt(movmean(gyro_x.^2, window_size)); % RMS moving window
    ...
    xlabel('Waktu (s)');
    ylabel('Gyro X (°/s)');
    title('Sinyal Gyro X (Rotasi Pitch)');
    grid on;
    ...
    xlabel('Waktu (s)');
    ylabel('Nilai RMS');
    title('Nilai RMS dari Gyro X terhadap Waktu');
