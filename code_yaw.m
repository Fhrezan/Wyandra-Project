% ANALISIS GYRO Y (Rotasi Yaw) DENGAN RMS
try
    % Load file .mat
    load('S1P1_G.mat');
    ...
    % --- Ambil data ---
    gyro_y = gyroY;           % Rotasi yaw (sumbu Y)
    t = timestamps;           % Waktu
    ...
    rms_values = sqrt(movmean(gyro_y.^2, window_size)); % RMS moving window
    ...
    xlabel('Waktu (s)');
    ylabel('Gyro Y (°/s)');
    title('Sinyal Gyro Y (Rotasi Yaw)');
    grid on;
    ...
    xlabel('Waktu (s)');
    ylabel('Nilai RMS');
    title('Nilai RMS dari Gyro Y terhadap Waktu');
